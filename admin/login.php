	...
	if(mysqli_num_rows($query)>0)
			{
				...
				if($role=='volunteer')
				{
					$role_id=mysqli_query($conn,"SELECT * FROM volunteers WHERE login_id='$row[login_id]'");
				...
				{
					$role_id=mysqli_query($conn,"SELECT status FROM organizer WHERE login_id='$row[login_id]'");
					if(mysqli_num_rows($role_id)>0){
						$checking_appoval=mysqli_fetch_assoc($role_id);
					     
						if($checking_appoval['status']==1)
						  {
							 $_SESSION["user_id"]=$row['login_id'];
						   $_SESSION["type_id"]=$row['type'];
							header("location: index.php");	
